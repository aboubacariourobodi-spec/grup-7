...
if(isset($_POST['submit'])){
    ...
    $sifre = ($_POST['sifre']);
    $sifreOnayla = ($_POST['sifreOnayla']);
    if (empty($adiSoyadi) || empty($e_posta) || empty($sifre)) {
        $error = "Tüm alanlar zorunludur.";
    } elseif ($sifre !== $sifreOnayla) {
        ...
        $sql = "select * from kullanici where e_posta = '$e_posta ' ";
    $result = mysqli_query($conn,$sql);
    $count_e_posta=mysqli_num_rows($result);

    if($count_e_posta==0){
        if($sifre==$sifreOnayla){
            $hash =password_hash($sifre,PASSWORD_DEFAULT);
            $sql = "insert into kullanici(adiSoyadi,e_posta,sifre) values ('$adiSoyadi','$e_posta','$hash')";
            $result=mysqli_query($conn,$sql);
        }
        else{
            echo '<script>
...
<title>ÖPSP - Kullanıcı Kaydı</title>
...
<div class="relative flex h-full min-h-screen w-full flex-col overflow-x-hidden">
    <div class="flex items-center justify-between p-4">
        <button class="flex items-center justify-center p-2 rounded-full hover:bg-black/5 dark:hover:bg-white/10 transition-colors">
            <span class="material-symbols-outlined text-slate-900 dark:text-white text-2xl"><a href="index.html">arrow_back</a></span>
        </button>
    ...
    <main class="flex-1 flex flex-col px-6 pt-2 pb-8 max-w-md mx-auto w-full">
        <div class="mb-8">
            <h1 class="text-3xl font-bold tracking-tight text-slate-900 dark:text-white mb-2">Hesap Oluştur</h1>
        ...
        <form class="flex flex-col gap-5" action="register.php" method="post" >
            ...
            <div class="flex flex-col gap-2">
                <label class="text-sm font-medium text-slate-900 dark:text-white" for="password">Şifre Onayla</label>
                <div class="relative flex items-center">
                    <input class="form-input flex w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder:text-slate-400 h-14 px-4 pr-12 focus:border-amber-400 focus:ring-amber-200 transition-all shadow-sm" id="password" placeholder="••••••••" type="password" name="sifreOnayla"/>
                    <button class="absolute right-4 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors" type="button">
                        <span class="material-symbols-outlined text-[20px]">visibility_off</span>
                    </button>
                </div>
            ...
            <button class="mt-2 w-full bg-primary hover:bg-amber-200 active:bg-amber-300 text-amber-900 font-bold h-14 rounded-xl flex items-center justify-center transition-all shadow-md hover:shadow-lg focus:ring-4 focus:ring-amber-200/50" 
        ...
        <div class="flex items-center justify-center gap-1">
            <span class="text-slate-600 dark:text-slate-400">Zaten bir hesabınız var mı?</span>
            <a class="font-bold text-amber-600 hover:text-amber-700 transition-colors" href="login.php">
                            Giriş Yap
            </a>
        </div>
    </main>
