...
<body class="font-display antialiased text-slate-900 dark:text-white bg-background-light dark:bg-background-dark flex flex-col h-screen overflow-hidden">
    ...
    <div class="flex-1 flex flex-col md:ml-64">
        ...
        <main class="flex flex-col lg:flex-row gap-6 p-6 w-full h-full overflow-hidden">
            <!-- LEFT: UPLOAD FORM -->
            <div class="w-full lg:w-1/3 bg-white dark:bg-surface-dark rounded-xl shadow p-6 overflow-y-auto">
                <h2 class="text-2xl font-bold mb-6">Yeni Sunum Yükle</h2>

                <?php if ($success): ?>
                    <div class="bg-green-100 text-green-700 p-4 rounded-xl mb-4"><?= htmlspecialchars($success) ?></div>
                <?php endif; ?>
                <?php if ($error): ?>
                    <div class="bg-red-100 text-red-700 p-4 rounded-xl mb-4"><?= htmlspecialchars($error) ?></div>
                <?php endif; ?>

                <form class="flex flex-col gap-4" method="post" action="" enctype="multipart/form-data">
                    <div class="flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-amber-300 bg-amber-50 dark:bg-amber-900/20 px-6 py-8 text-center">
                        
                       
                        <p class="text-sm text-gray-500">PDF, PPTX (Max 50MB)</p>
                    </div>

                    <input type="file" name="sunum_dosya" id="fileInput" accept=".pdf,.ppt,.pptx" required class="hidden">

                    <button type="button" onclick="document.getElementById('fileInput').click()"
                            class="h-12 rounded-xl bg-primary-dark text-white font-bold hover:brightness-110 transition">
                        Dosya Seç
                    </button>

                    <input name="baslik" required placeholder="Sunum Başlığı" 
                           class="h-12 rounded-xl px-4 border border-slate-300 dark:border-slate-700 bg-transparent">

                    <select name="kategori" required class="h-12 rounded-xl px-4 border border-slate-300 dark:border-slate-700 bg-transparent">
                    ...
                    <textarea name="aciklama" rows="4" placeholder="Sunum hakkında kısa açıklama (opsiyonel)"
                              class="rounded-xl p-4 border border-slate-300 dark:border-slate-700 bg-transparent resize-none"></textarea>

                    <button type="submit" name="submit"
                            class="h-14 rounded-xl bg-primary text-primary-text font-bold hover:brightness-95 transition shadow-lg">
                        Sunumu Yükle
                    </button>
                </form>
            </div>
